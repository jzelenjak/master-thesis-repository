...
# NOTE: The logs generated by UERANSIM use UTC time
...
parser = argparse.ArgumentParser(description="Plot the UE RLS UDP and RRC connections in the gNB over time")
    ...
    with open(exp_file_name, "r") as exp_file:
        for line in exp_file.readlines()[1:]:  # INFO: Skip the header
            ...
            if not protocol_name in protocols:
                ...
                protocols[protocol_name] = {"elapsed_seconds": [], "connections": []}
            ...
            protocols[protocol_name]["connections"].append(int(parts[2].strip()))
...
stats = {
    "rls-udp": { "title": "Active RLS UDP connections", "color": "red", "unit": "Number of connections" },
...
for i, protocol_name in enumerate(protocols.keys()):
    ...
    # INFO: Plot the number of connections for each experiment 
    for index in range(len(experiments)):
        ...
        color = stats[protocol_name]["color"] if len(experiments) == 1 else exp_colors[index]
    ...
    ax.set_ylabel(stats[protocol_name]["unit"], fontsize=FONT_SIZE_TEXT)
    ...
    ax.set_title(stats[protocol_name]["title"], fontsize=FONT_SIZE_SUBTITLE)
    ax.grid()

fig.suptitle(f"Number of UE connections stored in the gNB during the flooding attack over time", fontsize=FONT_SIZE_TITLE)

plt.gcf().set_size_inches(20, 11, forward=True)
plt.tight_layout()

plt.savefig(f"connections_gnodeb.{OUTPUT_FILE_EXTENSION}")
plt.show()
plt.close()
